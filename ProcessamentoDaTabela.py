...
def TabelaOriginal(emp=None):
    ...
    comp_boleto_receita_venda_total = """
    WITH rec_agg AS (                 
        SELECT 
            Fat.MesBase,
            Fat.NomeShopping,
            Fat.NumLoja,
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Aluguel Mínimo'               THEN Fat.ValorFaturado ELSE 0 END) AS [Aluguel Mínimo],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'F.Reserva Enc.Comum'          THEN Fat.ValorFaturado ELSE 0 END) AS [F.Reserva Enc.Comum],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Seguro Obrigatório'           THEN Fat.ValorFaturado ELSE 0 END) AS [Seguro Obrigatório],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Encargo Comum'                THEN Fat.ValorFaturado ELSE 0 END) AS [Encargo Comum],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Água/Esgoto'                  THEN Fat.ValorFaturado ELSE 0 END) AS [Água/Esgoto],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Energia'                      THEN Fat.ValorFaturado ELSE 0 END) AS [Energia],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Fundo de Promoção'            THEN Fat.ValorFaturado ELSE 0 END) AS [Fundo de Promoção],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Merchandising'                THEN Fat.ValorFaturado ELSE 0 END) AS [Merchandising],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'I.P.T.U.'                     THEN Fat.ValorFaturado ELSE 0 END) AS [I.P.T.U.],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Aluguel Percentual'           THEN Fat.ValorFaturado ELSE 0 END) AS [Aluguel Percentual],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Aluguel Min (Dif.)'           THEN Fat.ValorFaturado ELSE 0 END) AS [Aluguel Min (Dif.)],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Ar Condicionado'              THEN Fat.ValorFaturado ELSE 0 END) AS [Ar Condicionado],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Aluguel Complementar'         THEN Fat.ValorFaturado ELSE 0 END) AS [Aluguel Complementar],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'FPP Contribuição Estatutária' THEN Fat.ValorFaturado ELSE 0 END) AS [FPP Contribuição Estatutária],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Seguro Parte Privativa'       THEN Fat.ValorFaturado ELSE 0 END) AS [Seguro Parte Privativa],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Multa EDNG (Empreendedor)'    THEN Fat.ValorFaturado ELSE 0 END) AS [Multa EDNG (Empreendedor)],
            SUM(CASE WHEN Fat.NomeDaClasseCont = 'Serviços'                     THEN Fat.ValorFaturado ELSE 0 END) AS [Serviços]
        FROM BI_Faturamento AS Fat
        WHERE Fat.Situacao NOT IN ('CANCELADA','CRIADA')  
        GROUP BY Fat.MesBase, Fat.NomeShopping, Fat.NumLoja 
    ),
    aud_agg AS (                      
        SELECT 
            Aud.MesBase,
            Aud.NomeShopping,             
            Aud.NumLoja,
            SUM(Aud.Valor) AS Vendas
        FROM BI_AuditoriaVendas AS Aud
        GROUP BY Aud.MesBase, Aud.NomeShopping, Aud.NumLoja 
    ),
    fat_dim AS (                      
        -- 1 linha por (MesBase, NomeShopping, NumLoja)
        SELECT MesBase, NomeShopping, NumLoja,
            MIN(NomeFantasia) AS NomeFantasia  -- ou use ROW_NUMBER() se quiser uma regra mais específica
        FROM BI_Faturamento
        WHERE Situacao NOT IN ('CANCELADA','CRIADA')
        GROUP BY MesBase, NomeShopping, NumLoja
    ),
    lojas_dim AS (
        -- 1 linha por (MesBase, NomeShopping, NumLoja)
        SELECT l.MesBase, l.NomeShopping, l.NumLoja,
            MAX(l.AreaLoja)     AS AreaLoja,     -- se houver múltiplos registros por mês/loja
            MAX(l.Localizacao)  AS Localizacao,
            MAX(l.Nivel)        AS Nivel
        FROM BI_Lojas l
        GROUP BY l.MesBase, l.NomeShopping, l.NumLoja
    )
    ...
    SELECT 
        ...
        r.[Aluguel Mínimo],
        r.[F.Reserva Enc.Comum],
        r.[Seguro Obrigatório],
        r.[Encargo Comum],
        r.[Água/Esgoto],
        r.[Energia],
        r.[Fundo de Promoção],
        r.[Merchandising],
        r.[I.P.T.U.],
        r.[Aluguel Percentual],
        r.[Aluguel Min (Dif.)],
        r.[Ar Condicionado],
        r.[Aluguel Complementar],
        r.[FPP Contribuição Estatutária],
        r.[Seguro Parte Privativa],
        r.[Multa EDNG (Empreendedor)],
        r.[Serviços],
        ...
        -- Subselects dos contratos permanecem iguais, usando as chaves de f
        (
            SELECT TOP (1) c.DescTipo
            FROM BI_ContratosLocacao c
            WHERE c.NumLoja      = f.NumLoja
            AND c.NomeShopping = f.NomeShopping
            ORDER BY
                CASE WHEN c.MesBase = f.MesBase THEN 0 ELSE 1 END,
                CASE WHEN COALESCE(c.DataInauguracao, DATEFROMPARTS(1900,1,1)) <= f.MesBase
                        AND f.MesBase < COALESCE(c.DtRescisao, DATEFROMPARTS(9999,12,31))
                    THEN 0 ELSE 1 END,
                CASE WHEN c.NomeFantasia COLLATE Latin1_General_CI_AI = f.NomeFantasia COLLATE Latin1_General_CI_AI THEN 0 ELSE 1 END,
                COALESCE(c.DataInauguracao, DATEFROMPARTS(1900,1,1)) DESC
        ) AS DescTipo,

        (
            SELECT TOP (1) c.RamoDeAtividade
            FROM BI_ContratosLocacao c
            WHERE c.NumLoja      = f.NumLoja
            AND c.NomeShopping = f.NomeShopping
            ORDER BY
        ...
        (
            SELECT TOP (1) c.DataInauguracao
            FROM BI_ContratosLocacao c
            WHERE c.NumLoja      = f.NumLoja
            AND c.NomeShopping = f.NomeShopping
            ORDER BY
    ...
    FROM fat_dim f
    LEFT JOIN rec_agg  r ON r.MesBase = f.MesBase AND r.NomeShopping = f.NomeShopping AND r.NumLoja = f.NumLoja  
    LEFT JOIN aud_agg  a ON a.MesBase = f.MesBase AND a.NomeShopping = f.NomeShopping AND a.NumLoja = f.NumLoja  
    LEFT JOIN lojas_dim l ON l.MesBase = f.MesBase AND l.NomeShopping = f.NomeShopping AND l.NumLoja = f.NumLoja
    ORDER BY f.NomeShopping, f.NumLoja;

    """

    with engine.connect() as conn:
        df = pd.read_sql(comp_boleto_receita_venda_total, conn)

    df.rename(columns = {'MesBase': 'Data', 'NomeShopping': 'Empreendimento', 'NumLoja': 'Luc', 'NomeFantasia': 'Nome Fantasia',
                        'Vendas': 'Venda', 'AreaLoja': 'M2', 'DataInauguracao': 'Data que entrou', 'DtRescisao': 'Data que saiu', 
                        'Nivel': 'Piso', 'Localizacao': 'Lado', 'DescTipo': 'Classificação', 'RamoDeAtividade': 'Atividade', 
                        'Fundo de Promoção': 'Fundo Promoção'}, inplace=True)
    df['ID'] = df['Luc']+'_'+df['Nome Fantasia']
    df['Data'] = pd.to_datetime(df['Data'])
    df = df[df['Data']>='2025-08-01']

    conn = sqlite.connect('banco_de_dados.db')

    with conn:
        df_bd = pd.read_sql('SELECT * FROM bd_lojas', conn)
    
    df_bd['Data'] = pd.to_datetime(df_bd['Data'], errors='coerce')
    df_bd = df_bd.sort_values(by='Data', ascending= True)
    df_bd = df_bd[df_bd['Data']<'2025-08-01']



    dataframe = pd.concat([df_bd,df], ignore_index=True)
    dataframe['Data'] = pd.to_datetime(dataframe['Data'])
    dataframe = dataframe.fillna(0)

    dataframe['Piso'] = dataframe['Piso'].str.strip()
    piso_map = {
        '1º Piso': 'PISO 1',
        '2º Piso': 'PISO 2',
        '3º Piso': 'PISO 3',
    ...
    class_map = {
        ...
        'Midia': 'Mídia',
        'Deposito': 'Depósito',
        'Semi ancora': 'Semi Âncoras',
        'Megaloja': 'Mega Lojas',
        'Midia inaugural': 'Mídia',
        'Midia patroc.': 'Mídia',
        'Painel de led': 'Mídia',
        'Serviços': 'Conveniência / Serviços'
    }
    ...
    dataframe['Classificação'] = dataframe['Classificação'].str.capitalize()
    dataframe['Segmento'] = dataframe['Segmento'].str.capitalize()
    dataframe['Atividade'] = dataframe['Atividade'].str.capitalize()
    dataframe['Piso'] = dataframe['Piso'].replace(piso_map)
    dataframe['Classificação'] = dataframe['Classificação'].replace(class_map)
    dataframe['Segmento'] = dataframe['Segmento'].replace(segmento_map)
    dataframe['Lado'] = dataframe['Lado'].replace(lado_map)
    dataframe = dataframe[['Data', 'Luc', 'Nome Fantasia', 'M2', 'Venda', 'Aluguel Mínimo', 'Aluguel Complementar', 
                        'Aluguel Percentual', 'Encargo Comum', 'F.Reserva Enc.Comum', 'Fundo Promoção', 'I.P.T.U.', 
                        'Água/Esgoto', 'Ar Condicionado', 'Energia', 'Multa EDNG (Empreendedor)', 'Seguro Parte Privativa', 
                        'ID',	'Classificação', 'Segmento', 'Atividade', 'Data que entrou', 'Data que saiu', 'Piso', 
                        'Lado', 'Empreendimento', 'Seguro Obrigatório', 'Merchandising', 'Aluguel Min (Dif.)', 'Serviços'
                        ]]
    datas = ['Data', 'Data que entrou', 'Data que saiu']
    for col in datas:
        dataframe[col] = pd.to_datetime(dataframe[col], errors="coerce")
    
    CompClassPos = dataframe
